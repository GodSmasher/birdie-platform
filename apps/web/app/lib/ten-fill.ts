...
// ── Installer (tenant) ──────────────────────────────────────────────────────
const INSTALLER = {
  name:    process.env.INSTALLER_COMPANY   || 'SunPeak Solar GmbH',
  street:  process.env.INSTALLER_ADDRESS   || 'Musterstraße 1',
  plzOrt:  process.env.INSTALLER_PLZORT    || '10115 Berlin',
...
export async function fillAN005(project: ProjectData, customer: string): Promise<Uint8Array> {
  ...
  // ── 3. ausführender Elektrofachbetrieb = Installer ──
  text('Vorname Name bzw Firmenname_2', INSTALLER.name);
  text('Telefon_2', INSTALLER.phone);
  text('Straße Hausnummer_3', INSTALLER.street);
  text('PLZ Ort_3', INSTALLER.plzOrt);
  text('EMail_2', INSTALLER.email);
  text('Ausweisnummer', INSTALLER.ausweis);   // HWKN/IHK-Ausweisnr des Elektrofachbetriebs

  // ── 4. ausführender Anlagenerrichter (falls abweichend von 3) ──
  // Installer = beides, also gleiche Daten oder leer. Wir füllen gleich.
  text('Vorname Name bzw Firmenname_3', INSTALLER.name);
  text('Telefon_3', INSTALLER.phone);
  text('Straße Hausnummer_4', INSTALLER.street);
  text('PLZ Ort_4', INSTALLER.plzOrt);
  text('EMail_3', INSTALLER.email);
...
export async function fillANS(project: ProjectData, customer: string): Promise<Uint8Array> {
  ...
  // ── Abschnitt 7: Errichter = Installer ──
  text('Firmenname', INSTALLER.name);
  text('Eingetragen bei NB', INSTALLER.nbReg);        // Eintragungsnummer bei TEN
  text('Straße und HausNr_3', INSTALLER.street);
  text('Ausweisnummer', INSTALLER.ausweis);            // HWKN/IHK-Ausweisnummer
  text('Postleitzahl Ort_5', INSTALLER.plzOrt);
  text('Telefon Fax EMail_3', [INSTALLER.phone, INSTALLER.email].filter(Boolean).join(' / '));
  text('Name in Druckschrift_3', INSTALLER.name);      // Errichter-Unterschrift Name
