  ...
  return (
    <>
      ...
      <main className="flex-1 min-w-0 flex flex-col bg-bg">
        <TopBar title={t('Reports')} subtitle={t('Performance overview · this month')} />
        <div className="flex-1 px-8 py-6 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Card className="overflow-hidden">
              <CardHeader title={t('Revenue')} right={<span className="text-xs text-fg3">{t('6-month trend')}</span>} />
              <div className="p-5">
                <svg viewBox={`0 0 ${chartW} ${chartH}`} className="w-full" role="img" aria-label={t('Revenue line chart')}>
                  <defs>
                  ...
                  <text x={chartW - padR + 4} y={goalY + 3} fontSize="9" fill="#FACC15" opacity="0.7">$115K {t('goal')}</text>
                  ...
                  {/* Month labels */}
                  {revMonths.map((m, i) => (
                    <text key={m} x={cx(i)} y={chartH - 6} textAnchor="middle" className="text-fg3" fill="currentColor" fontSize="11">{t(m)}</text>
                  ))}
                </svg>
              </div>
            </Card>
            ...
            {/* Lead sources donut */}
            <Card className="overflow-hidden">
              <CardHeader title={t('Lead Sources')} right={<span className="text-xs text-fg3">247 {t('total leads')}</span>} />
            ...
            {/* Pipeline by stage */}
            <Card className="overflow-hidden">
              <CardHeader title={t('Pipeline by Stage')} right={<span className="text-xs text-fg3">{usd(sv.reduce((a, b) => a + b, 0))} {t('total')}</span>} />
              <div className="p-5 flex flex-col gap-3">
                {pipeStages.map((stage, i) => (
                  <div key={stage} className="flex items-center gap-3">
                    <span className="w-[110px] text-xs text-fg2 shrink-0 truncate">{t(stage)}</span>
            ...
            {/* Crew utilization */}
            <Card className="overflow-hidden">
              <CardHeader title={t('Crew Utilization')} right={<span className="text-xs text-fg3">{t('40h capacity')}</span>} />
              <div className="p-5 flex flex-col gap-3">
                {crew6.map((c, i) => (
            ...
            <Card className="overflow-hidden col-span-2">
              <CardHeader title={t('IC Turnaround by Utility')} />
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-line text-fg3">
                      <th className="text-left font-medium px-5 py-3 tracking-[0.12em]">{t('UTILITY')}</th>
                      <th className="text-left font-medium px-5 py-3 tracking-[0.12em]">{t('AVG')}</th>
                      <th className="text-left font-medium px-5 py-3 tracking-[0.12em]">{t('APPS')}</th>
                      <th className="text-left font-medium px-5 py-3 tracking-[0.12em]">{t('ON-TIME')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {icTurnData.map(row => (
