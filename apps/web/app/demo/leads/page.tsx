  ...
  return (
    <>
      ...
      <main className="flex-1 min-w-0 flex flex-col bg-bg">
        <TopBar title={t('Leads')} subtitle={t('New prospects · before they become deals')} />
        <div className="flex-1 px-8 py-6 overflow-y-auto">

          {/* Lead count */}
          <div className="flex items-center gap-3 mb-5">
            <h2 className="text-[14px] font-semibold text-fg">247 {t('leads')}</h2>
          </div>
            ...
            <div className="flex gap-5 mt-3">
              {leadSources.map((s) => (
                <div key={s.label} className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] text-fg3 font-medium">{t(s.label)}</span>
              ...
              <tbody>
                {leads.map((lead, i) => (
                  ...
                  >
                    ...
                    {/* Status */}
                    <td className="px-4 py-2.5">
                      <Pill label={t(lead.status).toUpperCase()} tone={leadToneMap[lead.status]} dot />
                    </td>
                  </tr>
                ))}
              </tbody>
