...
import { config, credentialsFor, isMockMode } from './config.js';
...
async function main(): Promise<void> {
  ...
  if (isMockMode()) console.log('[netzbot] MOCK mode — sample jobs, no browser, no portal accounts (BIRDIE_MOCK=false for real runs)');
  else if (!config.birdieApiUrl) console.warn('[netzbot] BIRDIE_API_URL nicht gesetzt — Leerlauf');
  if (once) {
    await tick();
    return;
  }
