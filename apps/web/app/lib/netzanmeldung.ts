...
// Kanban columns in the CRM's PV installation workflow board.
// Column ids are workspace-specific — configure them via env.
// v3: Projects are identified via kanbanColumnId.
const NETZ_COLUMNS: Record<string, { label: string; startStatus: StageId }> = {
  [process.env.CRM_COLUMN_UTILITY_REQUEST || 'col-utility-request']: { label: 'NTS / Meter swap / Service entrance', startStatus: 'anfrage' },
  [process.env.CRM_COLUMN_COMMISSIONING || 'col-commissioning']: { label: 'Submit commissioning', startStatus: 'inbetriebnahme' },
};
