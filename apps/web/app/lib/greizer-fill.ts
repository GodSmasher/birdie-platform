...
export async function fillGREAna(project: ProjectData, customer: string): Promise<Uint8Array> {
  ...
  text('Straße und Hausnummer-1', INSTALLER.street);
  text('Postleitzahl Ort-2', INSTALLER.plzOrt);
  text('Telefon Fax EMail-1', [INSTALLER.phone, INSTALLER.email].filter(Boolean).join(' / '));
  text('Ausweisnummer', INSTALLER.ausweis);
