  ...
  return (
    <>
      ...
      <main className="flex-1 min-w-0 flex flex-col bg-bg h-screen overflow-hidden">
        <TopBar title={t('Projects')} subtitle={t('12 active installs · lead to PTO')} />
            ...
            <div className="grid grid-cols-4 gap-3">
              {projMetrics.map(m => (
                <Card key={m.k} className="p-5 flex flex-col gap-1">
                  <span className="text-[10px] font-medium text-fg2 tracking-[0.16em]">{t(m.k)}</span>
            ...
            {byStage.map(col => (
              <div key={col.name} className="flex flex-col min-w-[200px] bg-surface rounded-xl p-2 shrink-0">
                ...
                <div className="flex items-center gap-2 px-2 py-2">
                  ...
                  <span className="text-[11px] font-semibold text-fg truncate">{t(col.name)}</span>
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-[10px] text-fg2">
                          {p.kw} kW · {p.panels} {t('panels')} · {p.inverter}
                        </span>
