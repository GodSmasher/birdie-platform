// CLI to run a connector. Mock transport by default; BIRDIE_MOCK=false hits the real API.
...
async function main() {
  ...
  const ctx = createContext(id, { ...envConfig(id), ...config }, {
    retry: { retries: 3, timeoutMs: 10000 },
    logger: (m) => console.log('  ' + m),
  });

  console.log(`\n▸ ${connector.manifest.name} (${connector.manifest.id}) · ${isMockMode() ? 'mock transport' : 'LIVE API'}`);
