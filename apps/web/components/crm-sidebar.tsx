    ...
    <>
      ...
      {navDef.map(([group, items]) => (
        <div key={group} className="mb-5">
          {navOpen && (
            <p className="text-[9px] font-semibold text-fg4 tracking-[0.18em] uppercase mb-1.5 px-2.5">{t(group)}</p>
          )}
      ...
      <div className={`flex items-center gap-1 mb-2 ${navOpen ? 'px-2.5' : 'justify-center flex-col'}`} aria-label={t('Language')}>
      ...
      <div className={`flex items-center gap-2.5 py-2 rounded-lg hover:bg-surface transition-colors cursor-pointer ${navOpen ? 'px-2.5' : 'justify-center'}`}>
        ...
        {navOpen && (
          <div className="flex flex-col leading-tight">
            ...
            <span className="text-[10px] text-fg3">SunPeak Solar</span>
          </div>
        )}
      </div>
    </>
