...
export async function fillSNSpeicher(project: ProjectData, customer: string): Promise<Uint8Array> {
  ...
  // ── Errichter = Installer (mittlerer Block) ──
  text('Name Firma', INSTALLER.name);
  text('PLZ Ort', INSTALLER.plzOrt);
  text('Str HausNr_2', INSTALLER.street);
  text('Telefon 1', INSTALLER.phone);
  text('Telefon 2', INSTALLER.email);   // Feld heißt "Telefon 2", wird für Email genutzt
  ...
  // Kopplung: DC-gekoppelt mit Gleichrichter (Installer-Standard)
...
export async function fillSNIbn(project: ProjectData, customer: string): Promise<Uint8Array> {
  ...
  // ── Anlagenerrichter = Installer ──
  text('Firma', INSTALLER.name);
  text('Straße, Hausnr', INSTALLER.street);
  text('Telefon, E-Mail', [INSTALLER.phone, INSTALLER.email].filter(Boolean).join(' / '));
