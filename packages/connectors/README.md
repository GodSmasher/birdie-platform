...
> **Mock mode is the default.** `npm run test:connector -- <id>` runs the connector's real parsing code against a mock transport (`src/mock/fetch.ts`) — no keys, no network. Set `BIRDIE_MOCK=false` and the matching env vars to hit the live API.
