  ...
  return (
    <>
      ...
      <main className="flex-1 min-w-0 flex flex-col bg-bg">
        <TopBar title={t('Good morning, Sarah')} subtitle={t("Wednesday, July 8 · here's what needs you today")} />
            ...
            {dashKpis.map(k => (
              <Link key={k.k} href={k.link} className="bg-surface border border-line rounded-xl p-5 flex flex-col gap-2 hover:border-line-2 transition-colors">
                <div className="flex items-center">
                  <span className="font-medium text-[10px] text-fg2 tracking-[0.16em]">{t(k.k)}</span>
          ...
          <div className="grid grid-cols-[1fr_1fr] gap-4">
            ...
            <Card className="overflow-hidden">
              <div className="h-[52px] px-5 border-b border-line flex items-center">
                <h3 className="font-semibold text-sm text-fg">{t('Needs your attention')}</h3>
                ...
                {dashAttention.map((a, i) => (
                  <Link key={i} href={a.link} className={`flex items-start gap-3 px-5 py-3.5 hover:bg-surface-2 transition-colors ${i < dashAttention.length - 1 ? 'border-b border-line' : ''}`}>
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-[12px] text-fg2 leading-[18px]">{t(a.text)}</p>
                      <p className="text-[10px] text-fg3 mt-0.5">{t(a.meta)}</p>
                    </div>
                  </Link>
                ))}
              </div>
            ...
            <Card className="overflow-hidden">
              <div className="h-[52px] px-5 border-b border-line flex items-center">
                <h3 className="font-semibold text-sm text-fg">{t("Today's schedule")}</h3>
                <Link href="/demo/schedule" className="ml-auto text-[11px] text-accent font-medium">{t('View all')} →</Link>
              </div>
              <div className="flex flex-col">
                {dashToday.map((job, i) => {
                  const jk = jobKindMap[job.kind];
                  return (
                    <div key={i} className={`flex items-center gap-3 px-5 py-3 ${i < dashToday.length - 1 ? 'border-b border-line' : ''}`}>
                      <span className="text-[11px] text-fg3 w-11 shrink-0">{job.time}</span>
                      <div className={`shrink-0 w-7 h-7 rounded-lg flex items-center justify-center text-[10px] ${jk.bg} ${jk.color}`}>
                        {job.kind === 'install' ? '⚡' : job.kind === 'visit' ? '◎' : job.kind === 'inspect' ? '◔' : '⚙'}
                      </div>
                      <div className="min-w-0 flex-1">
                        <span className="text-[12px] font-medium text-fg">{t(job.label)} — {job.cust}</span>
                        <p className="text-[10px] text-fg3">{job.who}</p>
                      </div>
                    </div>
                  );
                })}
            ...
            <Card className="overflow-hidden">
              <div className="h-[52px] px-5 border-b border-line flex items-center">
                <h3 className="font-semibold text-sm text-fg">{t('Interconnection')}</h3>
                <Link href="/demo/interconnection" className="ml-auto text-[11px] text-accent font-medium">{t('Details')} →</Link>
              </div>
              <div className="flex items-center gap-5 px-5 py-4">
                {dashIcStrip.map(s => (
                  <div key={s.l} className="text-center">
                    <div className={`text-[22px] font-bold ${s.c}`}>{s.n}</div>
                    <div className="text-[10px] text-fg4 mt-0.5">{t(s.l)}</div>
                  </div>
                ))}
              </div>
            </Card>
            ...
            <Card className="overflow-hidden">
              <div className="h-[52px] px-5 border-b border-line flex items-center">
                <h3 className="font-semibold text-sm text-fg">{t('Pipeline')}</h3>
                <Link href="/demo/pipeline" className="ml-auto text-[11px] text-accent font-medium">{t('Details')} →</Link>
              </div>
                ...
                <div className="flex flex-col gap-1.5 flex-1">
                  {pipeStages.slice(0, 5).map((name, i) => (
                    <div key={name} className="flex items-center gap-2 text-[11px]">
                      ...
                      <span className="text-fg2 flex-1">{t(name)}</span>
            ...
            <Card className="col-span-2 overflow-hidden">
              <div className="h-[52px] px-5 border-b border-line flex items-center">
                <h3 className="font-semibold text-sm text-fg">{t('Recent activity')}</h3>
              </div>
              <div className="flex flex-col">
                {dashActivity.map((a, i) => (
                  <div key={i} className={`flex items-center gap-3 px-5 py-3 ${i < dashActivity.length - 1 ? 'border-b border-line' : ''}`}>
                    ...
                    <span className="text-[12px] text-fg2 flex-1">{t(a.text)}</span>
                    <span className="text-[10px] text-fg4">{t(a.src)}</span>
