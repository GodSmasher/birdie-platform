  ...
  return (
    <>
      ...
      <main className="flex-1 min-w-0 flex flex-col bg-bg">
        <TopBar title={t('Pipeline')} subtitle={t('17 open deals · $486K weighted')} />
        <div className="flex-1 px-4 lg:px-8 py-6 overflow-y-auto space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {pipeSummary.map(s => (
              <Card key={s.k} className="p-5 flex flex-col gap-1.5">
                <span className="font-medium text-[10px] text-fg2 tracking-[0.16em]">{t(s.k)}</span>
          ...
          {/* ── Funnel chart ── */}
          <Card className="p-5">
            <h3 className="font-semibold text-sm text-fg mb-4">{t('Pipeline funnel')}</h3>
            <div className="flex flex-col gap-2">
              {pipeStages.slice(0, 5).map((stage, i) => {
                const pct = total > 0 ? Math.max((sv[i] / total) * 100, 6) : 6;
                return (
                  <div key={stage} className="flex items-center gap-3">
                    <span className="w-[110px] shrink-0 text-[11px] text-fg2 truncate">{t(stage)}</span>
          ...
          <div className="flex items-center gap-1 bg-surface border border-line rounded-lg p-1 w-fit">
            {(['board', 'table', 'forecast'] as const).map(v => (
              ...
              >
                {v === 'board' ? t('Board') : v === 'table' ? t('Table') : t('Forecast')}
              </button>
            ))}
          </div>
              ...
              {cols.map((col, ci) => (
                <div key={col.name} className="min-w-[200px] w-[220px] shrink-0 bg-surface rounded-xl flex flex-col">
                  ...
                  <div className="px-3 py-3 flex items-center gap-2 border-b border-line">
                    ...
                    <span className="text-[12px] font-semibold text-fg truncate">{t(col.name)}</span>
                    ...
                    {col.deals.map(d => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 text-[10px] text-fg3 mt-1">
                          <span>{t('{n}d in stage').replace('{n}', String(d.days))}</span>
                          <span className="ml-auto truncate">{d.rep}</span>
                        </div>
                      </div>
                    ))}
                    {col.deals.length === 0 && (
                      <div className="text-[11px] text-fg4 text-center py-6">{t('No deals')}</div>
                    )}
                  </div>
                </div>
              ))}
                  ...
                  <thead>
                    <tr className="border-b border-line">
                      {['CUSTOMER', 'STAGE', 'VALUE', 'SYSTEM', 'REP', 'AGE'].map(h => (
                        <th key={h} className="px-5 py-3 text-[10px] font-semibold text-fg3 tracking-[0.16em]">{t(h)}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      <tr key={d.cust} onClick={() => setSelectedDeal(d)} className={`border-b border-line hover:bg-surface transition-colors cursor-pointer ${i === deals.length - 1 ? 'border-b-0' : ''}`}>
                        ...
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[12px] text-fg2">{t(pipeStages[d.stageIdx])}</span>
                          </div>
                        </td>
                        ...
                        <td className="px-5 py-3 text-[12px] text-fg3">{t('{n}d').replace('{n}', String(d.days))}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {pipeView === 'forecast' && (
            <Card className="p-5">
              <h3 className="font-semibold text-sm text-fg mb-5">{t('Revenue forecast · Jul — Dec')}</h3>
              <div className="flex items-end gap-3 h-[200px]">
                {forecast.map(f => (
                  <div key={f.m} className="flex-1 flex flex-col items-center gap-2 h-full justify-end">
                    ...
                    />
                    <span className="text-[11px] text-fg3">{t(f.m)}</span>
                  </div>
                ))}
              </div>
            </Card>
