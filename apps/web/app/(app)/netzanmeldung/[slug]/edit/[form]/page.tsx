...
export default function EditDocPage({ params }: { params: { slug: string; form: string } }) {
  ...
  const formPath = decodeURIComponent(params.form);
  const fullForm = formPath.startsWith('ai:') ? formPath : `ai:${formPath}`;
  const pdfUrl = `/api/netzanmeldung/document?offerId=${params.slug}&form=${encodeURIComponent(fullForm)}`;
  ...
  async function handleFreigeben() {
    ...
    // WhatsApp an Jan (Elektriker) mit Signing-Link
    const signUrl = `${window.location.origin}/sign`;
    const msg = `Hi Jan, neue Dokumente zum Unterschreiben bereit: ${signUrl}`;
    window.open(`https://wa.me/10000000000?text=${encodeURIComponent(msg)}`, '_blank');
