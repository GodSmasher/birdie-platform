...
//
// The installer uses pCloud EU (eapi.pcloud.com) to exchange documents between office
// and field electricians. The electrician signs PDFs on a tablet, then
// uploads the signed version back.
//
// Folder structure (already exists in the installer's pCloud):
...
// ── Tenant-specific folder IDs (set via env) ─────────────────────────────────────────
// These come from the existing pCloud structure.

export const PCLOUD_FOLDERS = {
  netzanmeldungen: Number(process.env.PCLOUD_FOLDER_REGISTRATIONS || 0),       // /Netzanmeldungen/
  anmeldung: Number(process.env.PCLOUD_FOLDER_SUBMISSIONS || 0),             // /Netzanmeldungen/Anmeldung/
...
export async function getUnterschriebenFolder(): Promise<number> {
  const id = await findSubFolder(PCLOUD_FOLDERS.anmeldung, 'Unterschrieben');
...
export async function getKlaerungFolder(): Promise<number> {
  const id = await findSubFolder(PCLOUD_FOLDERS.anmeldung, 'Klärung');
