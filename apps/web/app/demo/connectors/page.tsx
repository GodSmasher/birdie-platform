  ...
  return (
    <>
      ...
      <main className="flex-1 min-w-0 flex flex-col bg-bg h-screen overflow-hidden">
        <TopBar title={t('Connectors')} subtitle={`ReNew Solar Solutions · ${t('Integration hub')} · ${CONNECTORS.length} ${t('connectors')}`} />

        <div className="flex-1 flex flex-col overflow-y-auto">
          <div className="px-6 pt-5 pb-3 shrink-0">
            <div className="grid grid-cols-4 gap-3">
              <Card className="p-4 flex flex-col gap-1">
                <span className="text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('CONNECTED')}</span>
                <span className="text-[28px] font-semibold text-success leading-none">{connectedCount}</span>
                <span className="text-[11px] text-fg3">{t('active integrations')}</span>
              </Card>
              <Card className="p-4 flex flex-col gap-1">
                <span className="text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('AVAILABLE')}</span>
                <span className="text-[28px] font-semibold text-info leading-none">{CONNECTORS.filter(c => c.status === 'available').length}</span>
                <span className="text-[11px] text-fg3">{t('ready to connect')}</span>
              </Card>
              <Card className="p-4 flex flex-col gap-1">
                <span className="text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('DATA POINTS')}</span>
                ...
                <span className="text-[11px] text-fg3">{t('synced fields')}</span>
              </Card>
              <Card className="p-4 flex flex-col gap-1">
                <span className="text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('COMING SOON')}</span>
                <span className="text-[28px] font-semibold text-fg3 leading-none">{CONNECTORS.filter(c => c.status === 'coming_soon').length}</span>
                <span className="text-[11px] text-fg3">{t('in development')}</span>
              </Card>
            </div>
          </div>
          ...
          <div className="px-6 pb-3 flex items-center gap-2 shrink-0">
            {(['all', 'connected', 'available'] as const).map(f => (
              <button key={f} onClick={() => setFilter(f)} className={`px-3 py-1.5 rounded-lg text-[11px] font-medium transition-colors ${filter === f ? 'bg-accent text-white' : 'bg-surface border border-line text-fg2 hover:text-fg'}`}>
                {f === 'all' ? t('All') : f === 'connected' ? t('Connected') : t('Available')}
              </button>
            ))}
          </div>
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5">
                        <Tag label={t(CAT_LABEL[conn.category])} tone={CAT_TONE[conn.category]} />
                        <Pill label={t(STATUS_LABEL[conn.status])} tone={STATUS_TONE[conn.status]} dot={false} />
                      </div>
                    </div>
                  </div>
                  <p className="text-[10px] text-fg3 mb-2 leading-relaxed line-clamp-2">{t(conn.description)}</p>
                  {conn.status === 'connected' && (
                    <div className="flex items-center gap-3 text-[9px] text-fg4">
                      <span>{conn.dataPoints.length} {t('data points')}</span>
                      <span>·</span>
                      <span>{t(conn.syncFreq)}</span>
                    </div>
                  )}
                </Card>
              ))}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <Pill label={t(STATUS_LABEL[selectedConn.status])} tone={STATUS_TONE[selectedConn.status]} dot={false} />
                    </div>
                    <p className="text-[10px] text-fg3 mt-0.5">{selectedConn.id} · {t(CAT_LABEL[selectedConn.category])}</p>
                  </div>
                </div>
                <button onClick={() => setSelectedConn(null)} className="text-fg3 hover:text-fg text-[18px] w-7 h-7 flex items-center justify-center rounded-lg hover:bg-surface" aria-label={t('Close')}>×</button>
              </div>

              <div className="px-5 py-3 border-b border-line shrink-0">
                <p className="text-[11px] text-fg2 leading-relaxed">{t(selectedConn.description)}</p>
              </div>

              {selectedConn.status === 'connected' && (
                <div className="px-5 py-3 border-b border-line grid grid-cols-2 gap-3 shrink-0">
                  <div>
                    <p className="text-[9px] text-fg4 uppercase tracking-wider">{t('Last Sync')}</p>
                    <p className="text-[11px] text-fg font-medium">{selectedConn.lastSync !== '—' ? new Date(selectedConn.lastSync).toLocaleString(locale === 'de' ? 'de-DE' : 'en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : '—'}</p>
                  </div>
                  <div>
                    <p className="text-[9px] text-fg4 uppercase tracking-wider">{t('Sync Frequency')}</p>
                    <p className="text-[11px] text-fg font-medium">{t(selectedConn.syncFreq)}</p>
                  </div>
                </div>
              )}

              <div className="px-5 py-4 border-b border-line shrink-0">
                <p className="text-[9px] text-fg4 uppercase tracking-wider mb-2">{t('Data Points')}</p>
                <div className="flex flex-wrap gap-1.5">
                  {selectedConn.dataPoints.map(dp => (
                    <Tag key={dp} label={t(dp)} tone={selectedConn.status === 'connected' ? 'success' : 'neutral'} />
                  ))}
                </div>
              </div>
              ...
              <div className="px-5 py-4 mt-auto shrink-0">
                {selectedConn.status === 'connected' ? (
                  <div className="flex gap-2">
                    <button className="flex-1 h-8 bg-surface border border-line text-fg2 text-[11px] font-medium rounded-lg hover:border-line-2 transition-colors">{t('Sync Now')}</button>
                    <button className="flex-1 h-8 bg-surface border border-line text-fg2 text-[11px] font-medium rounded-lg hover:border-line-2 transition-colors">{t('Settings')}</button>
                  </div>
                ) : selectedConn.status === 'available' ? (
                  <button className="w-full h-8 bg-accent text-white text-[11px] font-medium rounded-lg hover:bg-accent/90 transition-colors">{t('Connect')}</button>
                ) : (
                  <button className="w-full h-8 bg-surface border border-line text-fg3 text-[11px] font-medium rounded-lg cursor-not-allowed" disabled>{t('Coming Soon')}</button>
                )}
              </div>
            </div>
          </>
