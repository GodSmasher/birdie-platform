  ...
  return (
    <>
      ...
      <main className="flex-1 min-w-0 flex flex-col bg-bg h-screen overflow-hidden">
        <TopBar
          title={t('Interconnection')}
          subtitle={t('28 utility applications · 3 need action')}
        />
            ...
            <div className="grid grid-cols-4 gap-3">
              {icSummary.map(s => (
                <Card key={s.k} className="px-4 py-3">
                  <p className="text-[9px] text-fg4 uppercase tracking-wider font-medium">{t(s.k)}</p>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-line bg-surface">
                    ...
                    <th className="px-4 py-2.5 text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('Customer')}</th>
                    <th className="px-4 py-2.5 text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('Utility')}</th>
                    <th className="px-4 py-2.5 text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('Application #')}</th>
                    <th className="px-4 py-2.5 text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('Filed')}</th>
                    <th className="px-4 py-2.5 text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('Status')}</th>
                    <th className="px-4 py-2.5 text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('Days')}</th>
                    <th className="px-4 py-2.5 text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('Docs')}</th>
                    <th className="px-4 py-2.5 text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('Assigned')}</th>
                  </tr>
                </thead>
                <tbody>
                  {filtered.map(row => (
                    ...
                    >
                      ...
                      {/* Status */}
                      <td className="px-4 py-2.5">
                        <Pill label={t(row.status).toUpperCase()} tone={icStatusTone[row.status] as any} dot={false} />
                      </td>
                      ...
                      <td className="px-4 py-2.5">
                        {row.docs ? (
                          <span className="text-[11px] text-success font-medium">✓ {t('Complete')}</span>
                        ) : (
                          <span className="text-[11px] text-error font-medium">⚠ {t('Missing')}</span>
                        )}
                      </td>
