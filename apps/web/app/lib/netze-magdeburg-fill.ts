...
const INSTALLER = {
  ...
  plz:     process.env.INSTALLER_PLZ       || '10115',
  ort:     process.env.INSTALLER_CITY      || 'Berlin',
...
export async function fillNMDatenblatt(project: ProjectData, customer: string): Promise<Uint8Array> {
  ...
  // ── Block 3: Errichter = Installer (y=515–462) ──
  text('1_3', INSTALLER.name);
  text('2_3', INSTALLER.street);
  text('3_3', INSTALLER.plz);
  text('4_3', INSTALLER.ort);
...
export async function fillNME8(project: ProjectData, customer: string): Promise<Uint8Array> {
  ...
  // ── Ort/Datum Errichter ──
  text('E8_Text55', INSTALLER.ort);                     // Ort Errichter
  text('E8_Text21', INSTALLER.name);                    // Firma Errichter Unterschrift
...
export async function fillNMInbe(project: ProjectData, customer: string): Promise<Uint8Array> {
  ...
  // ── Errichter = Installer ──
  text('Anlagenerrichter  Installateur', INSTALLER.name);
  text('Straße Hausnummer 1_2', INSTALLER.street);
  text('Straße Hausnummer 2_2', INSTALLER.plzOrt);
  text('Straße Hausnummer 3', [INSTALLER.phone, INSTALLER.email].filter(Boolean).join(' / '));
