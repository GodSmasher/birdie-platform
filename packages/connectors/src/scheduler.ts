...
import { RateGate, type RetryPolicy } from './http.js';
import { createContext, createTransport } from './context.js';
...
export async function runBatch(jobs: PollJob[], opts: RunOptions = {}): Promise<PollResult[]> {
  ...
  const fetchImpl = createTransport(retry);
  ...
  return mapPool(jobs, concurrency, async (job): Promise<PollResult> => {
    ...
    try {
      ...
      const ctx = createContext(job.connectorId, job.config, { fetch: fetchImpl });
