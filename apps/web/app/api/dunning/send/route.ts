...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    vars.firma_name = vars.firma_name || 'SunPeak Solar GmbH';

    const rendered = renderTemplate(template, vars);

    // ── Send via Brevo SMTP API ──────────────────────────────────────
    const brevoBody: Record<string, unknown> = {
      sender: { name: 'SunPeak Solar', email: 'office@sunpeak-solar.example' },
      replyTo: { name: 'SunPeak Solar', email: 'office@sunpeak-solar.example' },
