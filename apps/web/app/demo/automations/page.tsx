  ...
  return (
    <>
      ...
      <main className="flex-1 min-w-0 flex flex-col bg-bg">
        <TopBar
          title={t('Automations')}
          subtitle={`${liveBotCount} ${t('bots live')} · ${workflows.length} ${t('workflows')}`}
        />
        <div className="flex-1 px-8 py-6 overflow-y-auto space-y-8">
          ...
          <section>
            <div className="flex items-center gap-2 mb-3">
              <h2 className="text-[14px] font-semibold text-fg">{t('Active bots')}</h2>
              ...
              {bots.map((bot) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <Pill
                      label={bot.status === 'live' ? t('LIVE') : t('PAUSED')}
                      tone={bot.status === 'live' ? 'success' : 'warning'}
                      dot={false}
                    />
                  </div>
                  <p className="text-[10px] text-fg3 leading-relaxed">{t(bot.desc)}</p>
                  <div className="flex items-center gap-3 text-[10px] text-fg4 mt-auto pt-1">
                    <span>{t('Last run')}: {t(bot.last)}</span>
                    <span>{t('Success')}: {bot.rate}</span>
                    <span>{t('Runs')}: {bot.runs}</span>
                  </div>
                  ...
                  {expandedBot === bot.name && (
                    <div className="pt-2 mt-1 border-t border-line flex items-center gap-2">
                      ...
                      >
                        {bot.status === 'live' ? t('Pause') : t('Resume')}
                      </button>
                      <span className="text-[10px] text-fg4">
                        {bot.status === 'live' ? t('Bot is running normally') : t('Bot is currently paused')}
                      </span>
                    </div>
                  )}
                </Card>
          ...
          {/* -- Workflows -------------------------------------------- */}
          <section>
            <h2 className="text-[14px] font-semibold text-fg mb-3">{t('Workflows')}</h2>
