...
export default function IntegrationsPage() {
  ...
  const handleConnected = useCallback((name: string) => {
    ...
    setToast(`${name} ${t('connected successfully')}`);
  }, [t]);
  ...
  const handleDisconnect = useCallback((name: string) => {
    ...
    setToast(`${name} ${t('disconnected')}`);
  }, [t]);

  const handleSync = useCallback((name: string) => {
    setToast(`${name} ${t('synced successfully')}`);
  }, [t]);
    ...
    <>
      ...
      <main className="flex-1 min-w-0 flex flex-col bg-bg">
        <TopBar
          ...
          subtitle={`${totalConnected} ${t('of')} ${items.length} ${t('connected')}`}
        />
        <div className="flex-1 px-8 py-6 overflow-y-auto">
          ...
          <Card className="p-5 mb-5">
            ...
            <p className="text-[12px] text-fg3 mt-2">
              {totalConnected} {t('of')} {items.length} {t('integrations connected')}
            </p>
          </Card>
                ...
                {connected.map((integ) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-fg4">{t(integ.cat)}</p>
                    </div>
