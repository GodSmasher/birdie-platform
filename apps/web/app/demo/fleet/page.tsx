  ...
  return (
    <>
      ...
      <main className="flex-1 min-w-0 flex flex-col bg-bg h-screen overflow-hidden">
        <TopBar title={t('Fleet & Crews')} subtitle={`ReNew Solar Solutions · Nashville, TN · ${t('Field Operations')}`} />

        <div className="flex-1 flex flex-col overflow-y-auto">
          {/* KPIs */}
          <div className="px-6 pt-5 pb-3 shrink-0">
            <div className="grid grid-cols-5 gap-3">
              <KpiCard label={t('CREWS ACTIVE')} value={`${activeCrews}/${CREWS.length}`} sub={t('on job today')} valueColor="text-success" />
              <KpiCard label={t('FIELD STAFF')} value={String(totalMembers)} sub={`${CREWS.length} ${t('crews')}`} />
              <KpiCard label={t("TODAY'S JOBS")} value={String(todayJobs)} sub={`${completedToday} ${t('completed')}`} />
              <KpiCard label={t('VEHICLES')} value={`${CREWS.filter(c => c.vehicle.status === 'active').length}/${CREWS.length}`} sub={t('deployed')} valueColor="text-accent" />
              <KpiCard label={t('AVG WEEKLY HRS')} value={`${Math.round(CREWS.reduce((a, c) => a + c.weeklyHours, 0) / CREWS.length)}`} sub={t('per crew')} />
            </div>
          </div>

          {/* Crew Cards */}
          <div className="px-6 pb-3 shrink-0">
            <p className="text-[9px] font-semibold text-fg4 tracking-[0.18em] uppercase mb-2">{t('CREWS')}</p>
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <h3 className="text-[13px] font-semibold text-fg">{t(crew.name)}</h3>
                      <Pill label={t(crew.status.replace('_', ' ')).toUpperCase()} tone={STATUS_TONE[crew.status]} dot={false} />
                    </div>
                  ...
                  <div className="flex items-center gap-4 mb-3">
                    <div className="flex items-center gap-1.5">
                      <span className="text-[10px] text-fg3">{t('Lead:')}</span>
                      <span className="text-[11px] font-medium text-fg">{crew.lead}</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <span className="text-[10px] text-fg3">{t('Team:')}</span>
                      <span className="text-[11px] text-fg">{crew.members.length} {t('members')}</span>
                    </div>
                  </div>

                  <div className="flex items-center gap-2 mb-3">
                    <Tag label={crew.vehicle.type} tone="neutral" />
                    <Pill label={t(crew.vehicle.status).toUpperCase()} tone={VEHICLE_TONE[crew.vehicle.status]} dot={false} />
                  ...
                  {crew.currentJob ? (
                    <div className="bg-accent/5 border border-accent/20 rounded-lg px-3 py-2">
                      <div className="flex items-center gap-2 mb-1">
                        <Tag label={t(crew.currentJob.type).toUpperCase()} tone={JOB_TYPE_TONE[crew.currentJob.type]} />
                      ...
                      <p className="text-[9px] text-fg3 mt-1">{t(crew.currentJob.notes)}</p>
                    </div>
                  ) : (
                    <div className="bg-success/5 border border-success/20 rounded-lg px-3 py-2">
                      <p className="text-[11px] text-success font-medium">{t('Available')}</p>
                      <p className="text-[10px] text-fg3">
                        {crew.todayJobs.length > 0
                          ? t('Next: {customer} at {time}').replace('{customer}', crew.todayJobs[0].customer).replace('{time}', crew.todayJobs[0].timeSlot)
                          : t('No more jobs scheduled today')}
                      </p>
                    </div>
                  )}
                </Card>
          ...
          <div className="px-6 pb-6 shrink-0">
            <p className="text-[9px] font-semibold text-fg4 tracking-[0.18em] uppercase mb-2">{t('UPCOMING JOBS')}</p>
            <Card className="overflow-hidden">
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-line bg-surface">
                    <th className="px-4 py-2.5 text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('Job')}</th>
                    <th className="px-4 py-2.5 text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('Type')}</th>
                    <th className="px-4 py-2.5 text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('Date')}</th>
                    <th className="px-4 py-2.5 text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('Time')}</th>
                    <th className="px-4 py-2.5 text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('System')}</th>
                    <th className="px-4 py-2.5 text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('Notes')}</th>
                  </tr>
                </thead>
                <tbody>
                  {UPCOMING_JOBS.map(j => (
                    <tr key={j.id} className="border-b border-line hover:bg-surface transition-colors">
                      ...
                      <td className="px-4 py-2.5"><Tag label={t(j.type).toUpperCase()} tone={JOB_TYPE_TONE[j.type]} /></td>
                      <td className="px-4 py-2.5 text-[11px] text-fg">{new Date(j.scheduledDate).toLocaleDateString(locale === 'de' ? 'de-DE' : 'en-US', { weekday: 'short', month: 'short', day: 'numeric' })}</td>
                      <td className="px-4 py-2.5 text-[11px] text-fg2">{j.timeSlot}</td>
                      <td className="px-4 py-2.5 text-[11px] text-fg">{j.systemKw} kW</td>
                      <td className="px-4 py-2.5 text-[10px] text-fg3 max-w-[200px] truncate">{t(j.notes)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <>
            ...
            <div className="fixed top-0 right-0 h-full w-[480px] bg-bg border-l border-line shadow-2xl z-40 flex flex-col overflow-y-auto">
              <div className="px-5 py-4 border-b border-line flex items-center justify-between shrink-0">
                <div>
                  <div className="flex items-center gap-2">
                    <h2 className="text-[14px] font-semibold text-fg">{t(selectedCrew.name)}</h2>
                    <Pill label={t(selectedCrew.status.replace('_', ' ')).toUpperCase()} tone={STATUS_TONE[selectedCrew.status]} dot={false} />
                  </div>
                  <p className="text-[11px] text-fg3 mt-0.5">{t('Lead:')} {selectedCrew.lead} · {selectedCrew.id}</p>
                </div>
                <button onClick={() => setSelectedCrew(null)} className="text-fg3 hover:text-fg text-[18px] w-7 h-7 flex items-center justify-center rounded-lg hover:bg-surface" aria-label={t('Close')}>×</button>
              </div>

              {/* Team Members */}
              <div className="px-5 py-4 border-b border-line shrink-0">
                <p className="text-[9px] text-fg4 uppercase tracking-wider mb-2">{t('Team Members')}</p>
                <div className="flex flex-col gap-2">
                  {selectedCrew.members.map(m => (
                    <div key={m.name} className="flex items-center justify-between">
                      <div>
                        <span className="text-[11px] font-medium text-fg">{m.name}</span>
                        <span className="text-[10px] text-fg3 ml-2">{t(m.role)}</span>
                      </div>
              ...
              <div className="px-5 py-3 border-b border-line shrink-0">
                <p className="text-[9px] text-fg4 uppercase tracking-wider mb-1">{t('Vehicle')}</p>
                <div className="flex items-center gap-2">
                  ...
                  <Pill label={t(selectedCrew.vehicle.status).toUpperCase()} tone={VEHICLE_TONE[selectedCrew.vehicle.status]} dot={false} />
                </div>
              </div>

              {/* Today's Schedule */}
              <div className="px-5 py-4 border-b border-line shrink-0">
                <p className="text-[9px] text-fg4 uppercase tracking-wider mb-2">{t("Today's Schedule")}</p>
                  ...
                  <div className="flex flex-col gap-2">
                    {selectedCrew.todayJobs.map(j => (
                      <div key={j.id} className={`rounded-lg px-3 py-2 border ${j.status === 'in_progress' ? 'bg-accent/5 border-accent/20' : 'bg-surface border-line'}`}>
                        <div className="flex items-center gap-2 mb-1">
                          <Tag label={t(j.type).toUpperCase()} tone={JOB_TYPE_TONE[j.type]} />
                          <span className="text-[11px] font-medium text-fg">{j.customer}</span>
                          {j.status === 'in_progress' && <Pill label={t('IN PROGRESS')} tone="accent" dot={false} />}
                        </div>
                        <p className="text-[10px] text-fg3">{j.address} · {j.timeSlot}</p>
                        <p className="text-[9px] text-fg3 mt-1">{t(j.notes)}</p>
                      </div>
                    ))}
                  </div>
                ) : (
                  <p className="text-[11px] text-fg3">{t('No jobs scheduled')}</p>
                )}
              </div>

              {/* Stats */}
              <div className="px-5 py-4 shrink-0">
                <p className="text-[9px] text-fg4 uppercase tracking-wider mb-2">{t('This Week')}</p>
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-surface rounded-lg px-3 py-2">
                    <p className="text-[18px] font-bold text-fg">{selectedCrew.weeklyHours}h</p>
                    <p className="text-[10px] text-fg3">{t('Hours logged')}</p>
                  </div>
                  <div className="bg-surface rounded-lg px-3 py-2">
                    ...
                    <p className="text-[10px] text-fg3">{t('Jobs today')}</p>
                  </div>
                </div>
              </div>
            </div>
