...
const BOTS: Bot[] = [
  {
    id: 'B-01', name: 'NES Interconnection Bot', description: 'Fills NES DG interconnection PDF, emails to interconnection@utility.example, tracks response',
    ...
    actions: ['Fill NES DG Application PDF with project data', 'Attach single-line diagram + site plan', 'Email to interconnection@utility.example', 'Log submission in project timeline', 'Set 15-day follow-up reminder'],
    logs: [
  ...
  return (
    <>
      ...
      <main className="flex-1 min-w-0 flex flex-col bg-bg h-screen overflow-hidden">
        <TopBar title={t('Bots')} subtitle={`ReNew Solar Solutions · ${BOTS.length} ${t('bots configured')} · ${t('Automation Engine')}`} />

        <div className="flex-1 flex flex-col overflow-y-auto">
          <div className="px-6 pt-5 pb-3 shrink-0">
            <div className="grid grid-cols-4 gap-3">
              <Card className="p-4 flex flex-col gap-1">
                <span className="text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('BOTS LIVE')}</span>
                <span className="text-[28px] font-semibold text-success leading-none">{liveCount}</span>
                <span className="text-[11px] text-fg3">{t('of')} {BOTS.length} {t('configured')}</span>
              </Card>
              <Card className="p-4 flex flex-col gap-1">
                <span className="text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('RUNS TODAY')}</span>
                <span className="text-[28px] font-semibold text-fg leading-none">{totalRuns}</span>
                <span className="text-[11px] text-fg3">{t('across all bots')}</span>
              </Card>
              <Card className="p-4 flex flex-col gap-1">
                <span className="text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('SUCCESS RATE')}</span>
                <span className="text-[28px] font-semibold text-success leading-none">98.7%</span>
                <span className="text-[11px] text-fg3">{t('last 30 days')}</span>
              </Card>
              <Card className="p-4 flex flex-col gap-1">
                <span className="text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('TIME SAVED')}</span>
                <span className="text-[28px] font-semibold text-accent leading-none">~14h</span>
                <span className="text-[11px] text-fg3">{t('this week (estimated)')}</span>
              </Card>
            </div>
          </div>
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-[13px] font-semibold text-fg">{t(bot.name)}</h3>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <Tag label={t(bot.type).toUpperCase()} tone={TYPE_TONE[bot.type]} />
                      <Pill label={t(bot.status).toUpperCase()} tone={STATUS_TONE[bot.status]} dot={false} />
                    </div>
                  </div>
                  <p className="text-[10px] text-fg3 mb-3 leading-relaxed">{t(bot.description)}</p>
                  <div className="flex items-center gap-4 text-[10px]">
                    <span className="text-fg2"><span className="font-medium text-fg">{bot.runsToday}</span> {t('runs today')}</span>
                    <span className="text-fg2"><span className="font-medium text-fg">{bot.runsTotal}</span> {t('total')}</span>
                    {bot.successRate > 0 && <span className="text-success font-medium">{bot.successRate}% {t('success')}</span>}
                    {bot.lastRun !== '—' && <span className="text-fg4 ml-auto">{t('Last:')} {new Date(bot.lastRun).toLocaleTimeString(locale === 'de' ? 'de-DE' : 'en-US', { hour: '2-digit', minute: '2-digit' })}</span>}
                  </div>
                </Card>
              ))}
            </div>
          ...
          <>
            ...
            <div className="fixed top-0 right-0 h-full w-[480px] bg-bg border-l border-line shadow-2xl z-40 flex flex-col overflow-y-auto">
              <div className="px-5 py-4 border-b border-line flex items-center justify-between shrink-0">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-[14px] font-semibold text-fg">{t(selectedBot.name)}</h2>
                    <Pill label={t(selectedBot.status).toUpperCase()} tone={STATUS_TONE[selectedBot.status]} dot={false} />
                  </div>
                  <p className="text-[11px] text-fg3 mt-0.5">{selectedBot.id} · {t(selectedBot.type)}</p>
                </div>
                <button onClick={() => setSelectedBot(null)} className="text-fg3 hover:text-fg text-[18px] w-7 h-7 flex items-center justify-center rounded-lg hover:bg-surface" aria-label={t('Close')}>×</button>
              </div>

              <div className="px-5 py-3 border-b border-line shrink-0">
                <p className="text-[11px] text-fg2 leading-relaxed">{t(selectedBot.description)}</p>
              </div>

              <div className="px-5 py-3 border-b border-line grid grid-cols-3 gap-3 shrink-0">
                <div>
                  <p className="text-[9px] text-fg4 uppercase tracking-wider">{t('Today')}</p>
                  <p className="text-[16px] font-bold text-fg">{selectedBot.runsToday}</p>
                </div>
                <div>
                  <p className="text-[9px] text-fg4 uppercase tracking-wider">{t('Total')}</p>
                  <p className="text-[16px] font-bold text-fg">{selectedBot.runsTotal}</p>
                </div>
                <div>
                  <p className="text-[9px] text-fg4 uppercase tracking-wider">{t('Success')}</p>
              ...
              <div className="px-5 py-4 border-b border-line shrink-0">
                <p className="text-[9px] text-fg4 uppercase tracking-wider mb-2">{t('Triggers')}</p>
                <div className="flex flex-col gap-1.5">
                  {selectedBot.triggers.map((trigger, i) => (
                    <div key={i} className="flex items-center gap-2">
                      <span className="text-[10px] text-accent">⚡</span>
                      <span className="text-[11px] text-fg">{t(trigger)}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="px-5 py-4 border-b border-line shrink-0">
                <p className="text-[9px] text-fg4 uppercase tracking-wider mb-2">{t('Actions')}</p>
                <div className="flex flex-col gap-1.5">
                  {selectedBot.actions.map((a, i) => (
                    <div key={i} className="flex items-start gap-2">
                      <span className="text-[10px] text-fg3 shrink-0 w-4 text-right">{i + 1}.</span>
                      <span className="text-[11px] text-fg">{t(a)}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="px-5 py-4 flex-1">
                <p className="text-[9px] text-fg4 uppercase tracking-wider mb-2">{t('Recent Activity')}</p>
                  ...
                  {selectedBot.logs.map((log, i) => (
                    <div key={i} className="flex items-start gap-2.5 py-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <span className="text-[10px] text-fg">{t(log.event)}</span>
