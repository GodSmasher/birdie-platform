...
> **Public showcase note.** This repository ships the worker architecture, the shared Lovion driver family (`_lovion.ts`, `sw-suhl.ts`, `sw-bayreuth.ts`), one standalone driver (`ten.ts`) and a mock driver. The remaining drivers listed below live in the private repository. With no configuration the worker runs in **mock mode** (`npm run once`): sample jobs, no browser, no portal accounts. Set `BIRDIE_MOCK=false` plus `NETZBOT_CREDS_*` to run against real portals.
...
## Unterstützte Portale (42, private Implementierung)
