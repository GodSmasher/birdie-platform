...
import { useT } from '@/app/demo/i18n/client';
...
const WORKFLOWS: Workflow[] = [
  {
    ...
    steps: [
      ...
      { name: 'Email to NES', type: 'bot', description: 'NES Bot sends to interconnection@utility.example' },
  ...
  return (
    <>
      ...
      <main className="flex-1 min-w-0 flex flex-col bg-bg h-screen overflow-hidden">
        <TopBar title={t('Workflows')} subtitle={`ReNew Solar Solutions · ${t('Automation orchestration')} · ${WORKFLOWS.length} ${t('workflows')}`} />

        <div className="flex-1 flex flex-col overflow-y-auto">
          <div className="px-6 pt-5 pb-3 shrink-0">
            <div className="grid grid-cols-4 gap-3">
              <Card className="p-4 flex flex-col gap-1">
                <span className="text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('ACTIVE')}</span>
                <span className="text-[28px] font-semibold text-success leading-none">{WORKFLOWS.filter(w => w.status === 'active').length}</span>
                <span className="text-[11px] text-fg3">{t('workflows running')}</span>
              </Card>
              <Card className="p-4 flex flex-col gap-1">
                <span className="text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('IN PROGRESS')}</span>
                <span className="text-[28px] font-semibold text-accent leading-none">{WORKFLOWS.reduce((a, w) => a + w.runsActive, 0)}</span>
                <span className="text-[11px] text-fg3">{t('active runs')}</span>
              </Card>
              <Card className="p-4 flex flex-col gap-1">
                <span className="text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('COMPLETED')}</span>
                <span className="text-[28px] font-semibold text-fg leading-none">{WORKFLOWS.reduce((a, w) => a + w.runsTotal, 0)}</span>
                <span className="text-[11px] text-fg3">{t('total runs')}</span>
              </Card>
              <Card className="p-4 flex flex-col gap-1">
                <span className="text-[10px] text-fg4 uppercase tracking-wider font-medium">{t('TOTAL STEPS')}</span>
                <span className="text-[28px] font-semibold text-fg leading-none">{WORKFLOWS.reduce((a, w) => a + w.steps.length, 0)}</span>
                <span className="text-[11px] text-fg3">{t('automated actions')}</span>
              </Card>
            </div>
          </div>
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <h3 className="text-[13px] font-semibold text-fg">{t(wf.name)}</h3>
                    <Pill label={t(wf.status.toUpperCase())} tone={STATUS_TONE[wf.status]} dot={false} />
                  </div>
                  <span className="text-[10px] text-fg3">{wf.id}</span>
                </div>
                <p className="text-[10px] text-fg3 mb-3">{t(wf.description)}</p>
                ...
                <div className="flex items-center gap-4 text-[10px]">
                  <span className="text-fg2">{t('Trigger')}: <span className="text-fg font-medium">{t(wf.trigger)}</span></span>
                  <span className="text-fg4">·</span>
                  <span className="text-fg2">{wf.steps.length} {t('steps')}</span>
                  <span className="text-fg4">·</span>
                  <span className="text-fg2">{t('Avg')}: {t(wf.avgDuration)}</span>
                  {wf.runsActive > 0 && <><span className="text-fg4">·</span><span className="text-accent font-medium">{wf.runsActive} {t('active')}</span></>}
                </div>
              </Card>
            ))}
          </div>
          ...
          <>
            ...
            <div className="fixed top-0 right-0 h-full w-[520px] bg-bg border-l border-line shadow-2xl z-40 flex flex-col overflow-y-auto">
              <div className="px-5 py-4 border-b border-line flex items-center justify-between shrink-0">
                <div>
                  <div className="flex items-center gap-2">
                    <h2 className="text-[14px] font-semibold text-fg">{t(selectedWf.name)}</h2>
                    <Pill label={t(selectedWf.status.toUpperCase())} tone={STATUS_TONE[selectedWf.status]} dot={false} />
                  </div>
                  <p className="text-[11px] text-fg3 mt-0.5">{selectedWf.id} · {selectedWf.steps.length} {t('steps')} · {t('Avg')} {t(selectedWf.avgDuration)}</p>
                </div>
                <button onClick={() => setSelectedWf(null)} aria-label={t('Close')} className="text-fg3 hover:text-fg text-[18px] w-7 h-7 flex items-center justify-center rounded-lg hover:bg-surface">×</button>
              </div>

              <div className="px-5 py-3 border-b border-line shrink-0">
                <p className="text-[9px] text-fg4 uppercase tracking-wider mb-1">{t('Trigger')}</p>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-accent">⚡</span>
                  <span className="text-[11px] text-fg">{t(selectedWf.trigger)}</span>
                </div>
              </div>

              <div className="px-5 py-3 border-b border-line grid grid-cols-3 gap-3 shrink-0">
                <div>
                  <p className="text-[9px] text-fg4 uppercase tracking-wider">{t('Active Runs')}</p>
                  <p className="text-[16px] font-bold text-accent">{selectedWf.runsActive}</p>
                </div>
                <div>
                  <p className="text-[9px] text-fg4 uppercase tracking-wider">{t('Total Runs')}</p>
                  <p className="text-[16px] font-bold text-fg">{selectedWf.runsTotal}</p>
                </div>
                <div>
                  <p className="text-[9px] text-fg4 uppercase tracking-wider">{t('Avg Duration')}</p>
                  <p className="text-[16px] font-bold text-fg">{t(selectedWf.avgDuration)}</p>
                </div>
              </div>

              <div className="px-5 py-4 flex-1">
                <p className="text-[9px] text-fg4 uppercase tracking-wider mb-3">{t('Workflow Steps')}</p>
                  ...
                  {selectedWf.steps.map((s, i) => (
                    <div key={i} className="flex items-start gap-3">
                      ...
                      <div className="flex-1 pb-2">
                        <div className="flex items-center gap-2">
                          <span className="text-[11px] font-medium text-fg">{t(s.name)}</span>
                          <Tag label={t(s.type.toUpperCase())} tone={STEP_TONE[s.type]} />
                        </div>
                        <p className="text-[10px] text-fg3 leading-[14px] mt-0.5">{t(s.description)}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="px-5 py-3 border-t border-line shrink-0">
                <div className="flex gap-1.5">
                  <Tag label={`${selectedWf.steps.filter(s => s.type === 'bot').length} ${t('Bot')}`} tone="accent" />
                  <Tag label={`${selectedWf.steps.filter(s => s.type === 'human').length} ${t('Human')}`} tone="info" />
                  <Tag label={`${selectedWf.steps.filter(s => s.type === 'condition').length} ${t('Condition')}`} tone="warning" />
                  <Tag label={`${selectedWf.steps.filter(s => s.type === 'wait').length} ${t('Wait')}`} tone="neutral" />
                </div>
              </div>
            </div>
          </>
